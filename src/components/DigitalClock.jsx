...
export const DigitalClock = ({ height = 140, width = 300 }) => {
	...
	return (
		<div className="dark" id="clock" style={{ height, width }}>
			<div className="display">
				<div className="weekdays">
					{days.map((wod) => (
						<span
							key={wod}
							className={weekDay === wod ? "active" : ""}
						>
							{wod}
						</span>
					))}
				...
				<div className="digits">
					{clockFormatParse().map((c) =>
						c === ":" ? (
							<div key={c} className="dots"></div>
						) : (
							<div key={c} className={digitdayName[c]}>
								{digitdayNameView[c].map((v) => (
									<span key={v} className={v}></span>
								))}
							</div>
						),
					)}
